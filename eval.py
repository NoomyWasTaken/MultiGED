...
def main():
    ...
    with open(args.hyp, encoding='utf-8') as hyp_tsv, open(args.ref, encoding='utf-8') as ref_tsv:
    ...
    a, p, r, f = compute_fscore(tp, tn, fp, fn, args.beta)
    # Print the overall results.
    print("")
    print('{:=^46}'.format(" Token-Based Detection "))
    print("\t".join(["TP", "TN", "FP", "FN", "Acc", "Prec", "Rec", "F"+str(args.beta)]))
    print("\t".join(map(str, [tp, tn, fp, fn, a, p, r, f])))
    print('{:=^46}'.format(""))
    print("")
...
def compute_fscore(tp, tn, fp, fn, beta):
    a = float(tp+tn)/(tp+tn+fp+fn) if fp and fn else 1.0
